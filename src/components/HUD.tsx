      ...
      >
        <div className="space-y-2 md:space-y-4">
          ...
          {/* <div className="flex items-center gap-2">
            <motion.div
              className={`w-1.5 h-1.5 rounded-full ${status === "ONLINE" ? "bg-emerald-400" : "bg-amber-400"
                }`}
              animate={status === "ONLINE" ? { opacity: [0.5, 1, 0.5] } : {}}
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
            />
            <span className={`text-[10px] tracking-[0.2em] uppercase ${status === "ONLINE" ? "text-white/50" : "text-white/30"
              }`}>
              {status}
            </span>
          </div> */}
